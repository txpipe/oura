...
For real-time, human-inspection of the events, use the [watch command](/oura/v2/usage/watch).
...
- `--magic <magic>`: the magic number of the network you're connecting to. Possible values are `mainnet`, `preprod`, `preview`, `testnet`. If omitted, the value `mainnet` is used as default.
...
:::tip
`--magic` accepts `mainnet`, `preprod`, `preview`, `testnet`, or a raw numeric magic. For
`--since`, find a block's `<slot>,<hash>` in any chain explorer — or omit it to start from the
tip. See the [watch command](/oura/v2/usage/watch) for the same options with a live, colorized
view.
:::
