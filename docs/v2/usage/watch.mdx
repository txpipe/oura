...
:::tip
`--magic` accepts `mainnet`, `preprod`, `preview`, `testnet`, or a raw numeric magic for a
custom network. For `--since`, the `<slot>,<hash>` of any block is shown by chain explorers
(e.g. [Cardanoscan](https://cardanoscan.io/)) — or just omit it to start from the tip.
:::
