...
- `--cursor`: a custom point in the chain to use as starting point. Expects format: `slot,hex-hash`. If not specified, it will look for the [cursor](/oura/v1/advanced/stateful_cursor) section available via toml configuration or fallback to the [intersect options](/oura/v1/advanced/intersect_options) of the source stage.
